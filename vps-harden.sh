#!/bin/bash
# Script to Harden Security on Ubuntu 16.04 LTS. Also tested on 18.04, 20.04 and 22.04 LTS. Based on the script made by AKcryptoguy.
...
function check_distro() {
    ...
    if [[ -r /etc/os-release ]]; then
        ...
        if [[ "${VERSION_ID}" != "16.04" ]] ; then
            echo -e "\nThis script works the very best with Ubuntu 16.04, 18.04 and 20.04 LTS."
            echo -e "Some elements of this script won't work correctly on other releases.\n"
        fi
    else
        # no, thats not ok!
        echo -e "This script does not support this OS, exiting.\n"
        exit 1
    fi
}
...
function update_upgrade() {
    ...
    # NOTE: characters "\" and "`" MUST be escaped with "\"
    echo -e -n "${lightcyan}"
...
function favored_packages() {
    ...
    echo '   update-motd unattended-upgrades secure-delete net-tools dnsutils build-essential g++' | tee -a "$LOGFILE"
    echo -e "------------------------------------------------- " | tee -a "$LOGFILE"
    echo -e -n "${nocolor}"
    apt-get -qqy -o=Dpkg::Use-Pty=0 -o=Acquire::ForceIPv4=true install \
        ...
        update-motd unattended-upgrades secure-delete net-tools dnsutils build-essential g++ | tee -a "$LOGFILE"
    echo -e -n "${lightgreen}"
    echo -e "----------------------------------------------------- " | tee -a "$LOGFILE"
...
function add_user() {
    ...
    echo -e " use an RSA key and continue to login as root. \n"
...
function collect_sshd() {
    ...
    while :; do
        ...
        [[ $SSHPORT =~ ^[0-9]+$ ]] || { echo -e -n "${lightred}";echo -e " --> Port must be a number. \n";echo -e -n "${nocolor}";continue; }
...
function google_auth() {
    ...
    echo -e " or any open source alternative like Aegis or OTPAuth. \n"
